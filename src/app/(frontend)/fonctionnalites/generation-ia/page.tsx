...
import { getPageBySlug, hasBlocks } from "@/lib/payload";
import RenderBlocks from "@/components/RenderBlocks";

const SITE_URL = "https://syllabis.fr";
const BREADCRUMB = [
  { name: "Accueil", url: `${SITE_URL}/` },
  { name: "Fonctionnalités", url: `${SITE_URL}/fonctionnalites` },
  { name: "Génération IA", url: `${SITE_URL}/fonctionnalites/generation-ia` },
];

const TITLE = "Génération IA | Syllabis, Du PDF au cours complet en 3 minutes";
const DESCRIPTION = "Chargez un référentiel RNCP et l'IA génère la formation complète : blocs, modules, séances, quiz. 6 certifications supportées.";
const URL_PATH = "/fonctionnalites/generation-ia";
...
export default async function GenerationIAPage() {
  const cmsPage = await getPageBySlug("fonctionnalites/generation-ia");
  return (
    <>
      <BreadcrumbJsonLd items={BREADCRUMB} />
      {hasBlocks(cmsPage) ? <RenderBlocks blocks={(cmsPage as any).blocks} /> : <HardcodedGenerationIAPage />}
    </>
  );
}

function HardcodedGenerationIAPage() {
  return (
    <>
      <ServiceJsonLd name={TITLE} description={DESCRIPTION} url={URL_PATH} />
      {/* ── Hero ── */}
      ...
      <CTABanner
        ...
        description="Créez votre compte gratuit et chargez votre référentiel : la génération démarre en quelques minutes."
        primaryLabel="Tester gratuitement"
